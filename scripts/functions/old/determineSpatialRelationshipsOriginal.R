...
DATABASE_NAME <- 'DaymetByHRDCatchments2'
...
for( t in 1:length(tilePoints) ){
  ...
  for ( year in years ){
    ...
    for( variable in variables ){
      ...
      for( k in 1:length(cats) ){
        ...
        for( f in 1:nrow(curCatch) ){
          ...
          indVar <- tileVar[curCatch$subRow[f], curCatch$subCol[f], 1:365]
          # Join them in an array
          if (f == 1) {
            tempVar <- data.frame(1:365, indVar)
          } else(tempVar <- cbind(tempVar, indVar))
        }
    ...
    print(paste0("Done writing results into database for ", year, " in tile #", t, " of ", length(unique(tilePoints$FEATUREID)), ". Elapsed time: ", (proc.time()[3] - S)/60, " minutes."))
    
    # This is what is running currently...
    # Actual number is number of Daymet points in the catchments
