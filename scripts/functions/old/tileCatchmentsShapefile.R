...
# shapefile - a spatial object of class SpatialPolygonsDataFrame 
tileCatchmentsShapefile <- function(shapefile){
  ...
  # If the shapefile fits into one tile then skip tiling, else perform tiling
  if( abs(EXT@xmin - EXT@xmax) <= 2 & abs(EXT@ymax - EXT@ymin) <= 2){
    
    shapefileTiles[[1]] <- shapefile
  ...
  } #end if statment: tile catchments
