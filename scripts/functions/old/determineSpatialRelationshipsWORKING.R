determineSpatialRelationships <- function(catchmentsShapefile, daymetDirectory, variables, years){
  ...
  for(t in 1:length(tiledShapefile) ){
    ...
    polygonPoints <- spatialJoin[which(!is.na(spatialJoin$FEATUREID)),]
    
    # Nearest Point
    # -------------
    # Determine the polygons without assigned points
    missingPolygons <- tile[!tile@data$FEATUREID %in% polygonPoints$FEATUREID,]
    
    missingCentroids <- data.frame(missingPolygons@data$FEATUREID, coordinates(missingPolygons) )
    names(missingCentroids) <- c('FEATUREID', 'LON', 'LAT')
    
    missingIDs <- missingPolygons@data$FEATUREID
    
    nearPoints <- as.data.frame(matrix(nrow = length(missingIDs), ncol = 3))
    names(nearPoints) <- c('FEATUREID', 'Longitude', 'Latitude')
    
    # Assign the nearest point to polygons missing point assignments
    for ( i in seq_along(missingIDs) ) {
      
      tempLat <- missingCentroids$LAT[missingCentroids$FEATUREID == missingIDs[i]]
      tempLon <- missingCentroids$LON[missingCentroids$FEATUREID == missingIDs[i]]
      
      distances <- spDistsN1(as.matrix(tilePoints), c(tempLon, tempLat), longlat = TRUE)
      minDist <- min(distances)
      distPos <- which(distances == minDist)[1]
      
      nearPoints$FEATUREID[i] <- missingIDs[i] 
    ...
    finalPoints <- rbind(polygonPoints[,c('FEATUREID', 'Longitude', 'Latitude')], nearPoints)
